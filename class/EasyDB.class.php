...
class EasyDB{
    ...
    public function select(){
        $this->sql = "SELECT {$this->options['field']} FROM {$this->options['table']}".$this->buildSql('where');
        $stmt = $this->pdo->prepare($this->sql);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    public function find(){
        $this->sql = "SELECT {$this->options['field']} FROM {$this->options['table']}".$this->buildSql('where')." LIMIT 1";
        $stmt = $this->pdo->prepare($this->sql);
        $stmt->execute();
    ...
    //字段
    public function filed($field=null){
    ...
    private function buildSql($type=null){
        switch($type){
            case 'where':
                ...
                if(!empty($where)){
                    ...
                    if(is_string($where)) {
                        ...
                        foreach($where as $k=>$v){
                            $whereStr .= is_array($v)?($k.' '.$v[0]." '".$v[1]."' and "): $k."='".$v."' and ";
                        }
    ...
    private function getFields(){
        $fields = array();
        $recordset = $this->dbh->query("SHOW COLUMNS FROM {$this->options['table']}");
        $this->getPDOError();
	...
	//查询总条目
	public function count2($table, $where = '', $data = array()){
	...
	public function delete2($table, $idata){
		$key = array_keys($idata);
		$where = '';
		foreach ($key as $v){
			$where .= '`'.$v.'`=? AND';
